with source as (
    select * from {{ source('weather_data', 'HISTORY_DAY') }}
),
...
renamed as (
    select
        postal_code,
        country,
        date_valid_std as observation_date,
        ...
        -- Add derived fields
        extract(year from date_valid_std) as observation_year,
        extract(month from date_valid_std) as observation_month,
        extract(day from date_valid_std) as observation_day
